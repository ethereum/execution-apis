...
- `valid`: `Boolean` - Whether the proof verification succeeded.
...
3. Client software **MUST** respond with `{valid: true}` if the proof is valid, or `{valid: false, error: "..."}` if the proof is invalid.
...
2. Client software **MUST** verify that sufficient execution proofs exist for the given `NewPayloadRequestHeader`. This involves:
   - Computing the `newPayloadRequestRoot` from the header.
   - Checking that the required number of valid proofs have been received for this root.

3. Client software **MUST** respond with `{valid: true}` if proof requirements are satisfied, or `{valid: false, error: "..."}` if not.
