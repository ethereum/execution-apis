...
2. Client software **MAY** skip an update of the forkchoice state if `forkchoiceState.headBlockHash` references a `VALID` but too distant ancestor of the head of canonical chain, i.e. the ancestor passed [payload validation](#payload-validation) process and deemed `VALID` but has a block number that is too far behind the head's block number. Client software **MUST** support a reorg to the head's ancestor if it is no more than `32` blocks behind the head, this limitation **MUST NOT** be applied to reorgs between different branches of the block tree. In the case of the skip, client software **MUST** return `{payloadStatus: {status: VALID, latestValidHash: forkchoiceState.headBlockHash, validationError: null}, payloadId: null}`.
