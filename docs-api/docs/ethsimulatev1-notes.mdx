...
| slotNumber | Previous block's `slotNumber` + 1. Omitted when the previous block has no `slotNumber` |
| blockAccessListHash | Calculated normally, see [Block access list](#block-access-list) |

## Block access list
After Amsterdam (EIP-7928) every block header carries a `blockAccessListHash`. A simulated block computes this hash from a block access list built with the EIP-7928 rules over the execution that the simulation performs. The following rules apply to the parts of a simulation that a real block does not have:

- State overrides (`nonce`, `balance`, `code`, `state`, `stateDiff`) are applied to the pre-state before the pre-execution system calls run. They are not recorded in the block access list at any index. They are reflected in `stateRoot`.
- `movePrecompileToAddress` changes no state and is not recorded. A call to the address that received the precompile records that address like any other call target.
- Pre-execution system calls (EIP-4788, EIP-2935) record at block access index `0`. Each call records at index `i + 1`. Post-execution system calls (EIP-7002, EIP-7251, EIP-8282) and the `withdrawals` from `blockOverrides` record at index `n + 1`, where `n` is the number of calls. This matches a real block.
- Each call executes as a transaction with the gas price and blob gas price fields that the simulation uses. With `validation` disabled these are zero by default. A zero fee produces no balance change. The coinbase is accessed during fee accounting in every block that has at least one call, whether or not `validation` is enabled. It is included with empty change lists, and gets a balance change only when the fee is not zero. A block with no calls does not include it.
- The sender nonce is incremented and recorded as a nonce change whether or not `validation` is enabled. Disabled validation skips the nonce check, not the increment.
- Blocks that the simulation inserts to fill a gap in block numbers are processed like every other block. Their block access list contains the system calls only.

Everything else follows EIP-7928 without change.
...
### EIP-7708
After Amsterdam the protocol emits a transfer log for every non-zero ETH transfer (EIP-7708). The log has the same topics and data as the `traceTransfers` log, but the emitting address is `0xfffffffffffffffffffffffffffffffffffffffe`. In blocks where EIP-7708 is active, `traceTransfers` adds no logs from `0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee`, and the flag has no effect. Blocks before Amsterdam in the same simulation still get the `0xeeee...` logs. A consumer that wants all ETH transfers filters on the `Transfer(address,address,uint256)` topic and accepts both emitting addresses.

Unlike the `traceTransfers` logs, the EIP-7708 logs are part of the receipts, so they count in `logsBloom` and `receiptsRoot`.
