...
`Transaction8141` contains the complete envelope fields. `from` represents the encoded sender. Fees are flat fields, and frames use `executionGasLimit` and `stateGasLimit`. `type`, modes, flags, and schemes use canonical hex quantities. The JSON type is `"0x6"`; the serialized transaction prefix is the byte `0x06`.
...
Chain ID, values, and fees fit unsigned 256-bit integers; nonce and individual frame budgets fit unsigned 64-bit integers. Blob hashes use version `0x01`. Without blobs, `maxFeePerBlobGas` is explicitly zero. Blob sidecars are transport data and are not part of this component.
...
```text
executionReservation = max(intrinsicGas + sum(executionGasLimit), calldataFloorGas)
stateReservation = sum(stateGasLimit)
...
The envelope rejects unknown properties, including outer `to`, `value`, `input`, `data`, `gas`, `gasPrice`, access lists, authorization lists, `sender`, and ECDSA `r`/`s`/`v`/`yParity`. Lookup metadata, compatibility response fields, partial preparation inputs, and method integration belong to subsequent changes.

Before merging, client maintainers must agree on `from`, `target`, flat gas limits and fees, hex quantities for modes/flags/schemes, explicit defaults, and signature metadata encoding. Null/zero compatibility fields and derived gas in responses remain separate decisions. This component does not settle simulation, estimation, receipts, or managed signing behavior.
