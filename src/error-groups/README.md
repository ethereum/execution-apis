...
## Implemented methods
Currently, only the below methods import the catalog error groups (Gas, Execution, TxPool) via `$ref`, and may include inline method-specific codes. The JSON-RPC standard/non-standard codes are not enumerated per-method (they are universal / implementation-defined).
...
| JSON-RPC non-standard | $-32099$ to $-32000$ | EIP-1474 (implementation-defined per JSON-RPC 2.0) |
...
1. Add or update a group file in `src/error-groups/` with `range` and `errors`.
