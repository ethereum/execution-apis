...
- Each call executes as a transaction with the gas price and blob gas price fields that the simulation uses. With `validation` disabled these are zero by default. A zero fee produces no balance change. The coinbase is accessed during fee accounting and is therefore included, with a balance change only when the fee is not zero.
